...
foodB_analytics = {"Values":[]}

# define a callback function to be called when a message is received
def foodB_callback(ch, method, properties, body, arguments = foodB_analytics["Values"]):
    """ Define behavior on getting a message."""
    # decode the binary message body to a string and process data by splitting the values by delimiter
    print(f" [x] Received reading {body.decode()}°F for FoodB")
    ...
    if (len(data_lead) > 0):
        ...
        if len(arguments) == 20:
            if -1 < float(data_lead) - float(tempA) < 1:
                print(f"ALERT: Temperature for FoodB has stalled!! < 1°F change for the previous 10 minutes")
...
if __name__ == "__main__":
    # call the main function, listening to queue 03, with the variables from v3_emitter_of_tasks
    main(v3_emitter_of_tasks.host, v3_emitter_of_tasks.queue_03)